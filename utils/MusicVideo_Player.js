...
const MusicVideoPlayer = ({ videoSrc }) => {
  ...
  const whilePlaying = () => {
    ...
    progressBar.current.style.setProperty('--seek-before-width',`${progressBar.current.value / totalDuration * 100 }%`)
      ...
      >
       ...
       <div className="flex-[0.6] w-full  flex flex-col justify-center items-center">
          ...
          <TbArrowsShuffle size={'15'} className="text-off-white text-[20px] "/>
            <TbPlayerTrackPrev size={'20'} className="text-[#fff] text-[20px] cursor-pointer" onClick={backThirty}/>
            {/*  */}
            ...
            {/*  */}
            <TbPlayerTrackNext size={'20'} className="text-[#fff] text-[20px] cursor-pointer " onClick={forwardThirty}/>
          ...
          <div className="player_idicator mb-5">
            ...
            <input ref={progressBar} type="range" className="player_idicator_position" onChange={progessBarHandler}  defaultValue={0}/>
            {/* total duration */}
            <article className="player_total_duration">{(totalDuration && !isNaN(totalDuration)) && formatTime(totalDuration) || '00:00'}</article>
          </div>
          {/* bottom */}
          <div className="flex flex-col justify-center items-center">
            <article className="text-off-white text-sm text-center"> BTS (방탄소년단) 'We are Bulletproof : the Eternal' MV</article>
          </div>
       </div>
