  ...
  return (
    <a href={`/stream/${music?.artist}/${music?.poster?.video}`}>
      ...
      >
        ...
        <div className="text-xs text-off-white flex justify-center items-center flex-[25%]">
          {formateNumber(artistInformation?.monthly_viewvers)}
        </div>

        <div className="py-2 flex-[25%] flex justify-end items-center gap-5">
          {false ? (
            <AiFillHeart className="text-xl text-green" />
          ) : (
            <AiOutlineHeart className="text-xl text-green" />
          )}
