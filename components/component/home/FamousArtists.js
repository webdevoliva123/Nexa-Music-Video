    ...
    <div className='w-full grid grid-cols-5 gap-5'>
      {
        Artists?.map((artist,idx) => {
          if(idx <= 4){
            return (
              <div key={idx} className='famous_artist_card relative w-full h-[250px] p-5 bg-[#222] bg-opacity-[0.5] hover:bg-opacity-[0.8] rounded-lg shadow-2xl cursor-pointer' onClick={() => router.push(`artist?id=${artist?.id}`)}>
